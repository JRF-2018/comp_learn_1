#!/usr/bin/python3
# -*- coding: utf-8 -*-
__version__ = '0.0.4' # Time-stamp: <2019-05-15T20:16:12Z>
...
for epoch in range(max_epoch):
    for iters in range(max_iters):
        ...
        if use_random_competitor:
            p3 = np.random.uniform(-2.0, 2.0, (batch_size, output_size))
        ...
        if epoch < max_epoch / 2:
            bs = neg_bs_first + (neg_bs_mid - neg_bs_first) * epoch / (max_epoch / 2)
        else:
            bs = neg_bs_mid + (neg_bs_last - neg_bs_mid) * (epoch - max_epoch / 2) / (max_epoch / 2)
        bs = bs * np.array([bsphere_rand(input_size) for i in range(batch_size)])
        
        pnd = neg_mid * (p1 - p2) + neg_coeff * (p1 - p2) * np.exp(- np.sum(np.square(p1 - p2), axis=1, keepdims=True) / (2 * neg_sigma ** 2)) + bs
        pn1 = pnd + p2
        pn2 = - pnd + p1
